...
public:
	...
	/* MEMBER VARIABLES */
	hitable *boundary; //Boundary of the volume
...
bool constant_medium::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	...
	if (boundary->hit(r, -FLT_MAX, FLT_MAX, rec1)) {
		if (boundary->hit(r, rec1.t + 0.0001, FLT_MAX, rec2)) {
			...
			if (hit_distance < distance_inside_boundary) { //Check if distance travelled by the ray before it hits goes beyond the volume
